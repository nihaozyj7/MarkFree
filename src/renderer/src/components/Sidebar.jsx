...
function Sidebar({ tabs, activeTabId, onSwitchTab, onOpenFile, folderFiles, onOpenFolderFile }) {
  const [activeSection, setActiveSection] = useState('opened')

  return (
    <div className="sidebar">
      <div className="sidebar-header">
        <button
          className={`sidebar-header-btn${activeSection === 'opened' ? ' active' : ''}`}
          onClick={() => setActiveSection('opened')}
        >
          已打开
        </button>
        <button
          className={`sidebar-header-btn${activeSection === 'folder' ? ' active' : ''}`}
      ...
      <div className="sidebar-content">
        {activeSection === 'opened' && (
          <div className="sidebar-list">
            <div className="sidebar-list-title">已打开的文件</div>
            {tabs.length === 0 ? (
        ...
        {activeSection === 'folder' && (
          <div className="sidebar-list">
            <div className="sidebar-list-title">
              <span>文件夹文件</span>
              <button className="sidebar-refresh-btn" onClick={onOpenFile} title="选择文件夹">
            ...
            {folderFiles.length === 0 ? (
              <div className="sidebar-empty">请先打开文件夹</div>
            ) : (
