      ...
      <div className={ `settings-dialog${open ? ' open' : ''}${closing ? ' closing' : ''}` } onClick={ e => e.stopPropagation() }>
        <div className="settings-header">
          ...
          <div className="settings-body">
              ...
              <div className="settings-group-body">
                <div className="settings-section">
                  ...
                  { themes.length === 0 ? (
                    ...
                    <select className="settings-select" value={ currentTheme } onChange={ e => onThemeChange(e.target.value) }>
                      { themes.map(t => (
                  ...
                  <div className="settings-section-actions">
                    <button className="settings-btn settings-btn-ghost" onClick={ () => window.electronAPI.openThemeFolder() }>打开主题文件夹</button>
                  </div>
                </div>
                <div className="settings-section">
                  ...
                  <div className="settings-row">
                    ...
                    <select className="settings-select settings-select-inline" value={ settings.fontFamily || 'default' } onChange={ e => updateSettings({ fontFamily: e.target.value }) }>
                  ...
                  <div className="settings-row">
                    ...
                    <select className="settings-select settings-select-inline" value={ settings.fontSize || 16 } onChange={ e => updateSettings({ fontSize: Number(e.target.value) }) }>
                ...
                <div className="settings-section">
                  ...
                  { Object.keys(settings.shortcuts || DEFAULT_SETTINGS.shortcuts).map(key => (
                    <div key={ key } className="settings-shortcut-row">
                      ...
                      <button className={ `settings-shortcut-key${editingShortcut === key ? ' recording' : ''}` } onClick={ () => setEditingShortcut(editingShortcut === key ? null : key) }>
                      ...
                      { (settings.shortcuts?.[key] && settings.shortcuts[key] !== DEFAULT_SETTINGS.shortcuts[key]) && (
                        <button className="settings-shortcut-reset" onClick={ () => updateSettings({ shortcuts: { ...settings.shortcuts, [key]: DEFAULT_SETTINGS.shortcuts[key] } }) } title="恢复默认">↺</button>
                      ) }
                    </div>
                  )) }
                </div>
            ...
            <div className="settings-group">
              <h4 className="settings-group-title">文件管理</h4>
              <div className="settings-group-body">
                <div className="settings-section">
                ...
                <div className="settings-section">
                  ...
                  <div className="settings-row">
                    ...
                    <select className="settings-select settings-select-inline" value={ folderSortMode?.split('-')[0] || 'foldersFirst' } onChange={ e => onFolderSortModeChange(e.target.value + '-' + (folderSortMode?.split('-')[1] || 'createTime')) }>
                  ...
                  <div className="settings-row">
                    ...
                    <select className="settings-select settings-select-inline" value={ folderSortMode?.split('-')[1] || 'createTime' } onChange={ e => onFolderSortModeChange((folderSortMode?.split('-')[0] || 'foldersFirst') + '-' + e.target.value) }>
                ...
                <div className="settings-section">
                  ...
                  <select className="settings-select" value={ settings.imageInsertMode } onChange={ e => updateSettings({ imageInsertMode: e.target.value }) }>
            ...
            <div className="settings-group">
              <h4 className="settings-group-title">窗口与启动</h4>
              <div className="settings-group-body">
                <div className="settings-section">
                  <h3 className="settings-section-title">硬件加速</h3>
                  <p className="settings-section-desc">修改后需重启应用生效</p>
                  <select className="settings-select" value={ hwAccel } onChange={ e => onHwAccelChange(e.target.value) }>
                ...
                <div className="settings-section">
                  ...
                  <select className="settings-select" value={ settings.closeLastTabAction } onChange={ e => updateSettings({ closeLastTabAction: e.target.value }) }>
                ...
                <div className="settings-section">
                  ...
                  <input className="settings-input" type="text" value={ defaultOpenPath } onChange={ e => onDefaultOpenPathChange?.(e.target.value) } placeholder="例如: C:\Users\Docs 或 C:\Users\Docs\readme.md" />
                </div>
                <div className="settings-section">
                  <h3 className="settings-section-title">窗口启动位置与尺寸</h3>
                  <p className="settings-section-desc">修改后需重启应用生效</p>
                  <select className="settings-select" value={ windowMode } onChange={ e => onWindowModeChange(e.target.value) }>
                  ...
                  { windowMode === 'fixed' && (
                    <div className="settings-window-bounds">
                      <div className="settings-row">
                        ...
                        <input className="settings-input settings-input-inline" type="number" value={ windowBounds?.x ?? 0 } onChange={ e => onWindowBoundsChange({ ...windowBounds, x: Number(e.target.value) }) } />
                      </div>
                      <div className="settings-row">
                        <label className="settings-row-label">Y</label>
                        <input className="settings-input settings-input-inline" type="number" value={ windowBounds?.y ?? 0 } onChange={ e => onWindowBoundsChange({ ...windowBounds, y: Number(e.target.value) }) } />
                      </div>
                      <div className="settings-row">
                        <label className="settings-row-label">宽</label>
                        <input className="settings-input settings-input-inline" type="number" min={ 680 } value={ windowBounds?.width ?? 1200 } onChange={ e => onWindowBoundsChange({ ...windowBounds, width: Number(e.target.value) }) } />
                      </div>
                      <div className="settings-row">
                        <label className="settings-row-label">高</label>
                        <input className="settings-input settings-input-inline" type="number" min={ 480 } value={ windowBounds?.height ?? 800 } onChange={ e => onWindowBoundsChange({ ...windowBounds, height: Number(e.target.value) }) } />
                      </div>
                    </div>
                  ) }
                </div>
